...
void TrapSetting(int StageNum)
{
	...
	case 0:
		...
		TrapManager::Instance().Register(new PoisonGus("./Data/Model/Floor.mdl", DirectX::XMFLOAT3(0.0f, 0.0f, 0.0f), DirectX::XMFLOAT3(0.01f, 0.01f, 0.01f)));
...
{
	stage_[StageNum].model = std::make_unique<Model>(StageModelPath[StageNum]);
	stage_collision_[StageNum].model = std::make_unique<Model>(CollisionModelPath[StageNum]);

	if (StageNum < 4)
	{
		stage_[StageNum].transform = stage_transform[StageNum];
		stage_collision_[StageNum].transform = stage_collision_transform[StageNum];
	}

	else
	{
		stage_[StageNum].transform = stage_transform[4];
		stage_collision_[StageNum].transform = stage_collision_transform[4];
	}

	SelectStage(StageNum);
	TrapSetting(StageNum);
}